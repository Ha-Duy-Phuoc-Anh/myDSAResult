...
public:
	...
	void push_at(int data, int position) {
		...
		// Nếu khác thì tạo nút tạm di chuyển đến trước ví trị mà người dùng đưa
		// ra
		Node* current = head;
		for (int i = 0; i < position - 1 && current != nullptr; i++)
			current = current->next;

		// Nếu current là nullptr thì xóa nút mới để tránh leak bộ nhớ (là
		// nullptr thì đã out of range)
		if (!current) {
			std::cerr << "ERROR 001: Invalid Position" << ENTER;
			delete newNode;
			return;
		...
		// Tạo nút tạm để giữ nút tiếp theo của nút current nếu nút tiếp theo
		// của current ko phải nullptr
	...
	int pop_front() {
		...
		// Nếu không thì tách head ra chỗ khác và xóa nó (lưu dữ liệu của nút
		// chuẩn bị xóa để trả về nữa)
	...
	int pop_back() {
		...
		// Lấy giá trị của nút đó, xóa nút đó đi và trả về giá trị của nút đã
		// xóa
	...
	int pop_at(int position) {
		...
		// Kiểm tra nếu current là nullptr (hoặc nút được xóa là nullptr) thì
		// báo lỗi
		...
		// Nếu ko thì tạo nút tạm tách nút được xóa với dữ liệu liệu của nút
		// được xóa ra Sau đó nối con trỏ curr vào nút tiếp theo của nút tiếp
		// theo và xóa nút tạm đó
