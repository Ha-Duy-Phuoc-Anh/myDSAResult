...
void BNode::remove(int key) {
	...
	if (idx < n && keys[idx] == key) {
		...
		// vì vậy chúng ta sẽ đệ quy trên nút con thứ (idx-1). Nếu không, chúng
		// ta sẽ đệ quy trên nút con thứ (idx) hiện có ít nhất t khóa.
		if (flag && idx > this->n)
...
void BNode::removeFromNonLeaf(int idx) {
	...
	// Thay thế key bằng 'successor' và đệ quy xóa successor trong children[idx
	// + 1]
	...
	// Nếu cả hai children[idx] và children[idx + 1] đều bé hơn các khóa t, ghép
	// key và tất cả children[idx + 1] vào children[idx]. Bây giờ children[idx]
	// chứa 2t - 1 khóa. Giải phóng bộ nhớ cho children[idx + 1] và đệ quy xóa
	// key trong children[idx]
	else {
		this->merge(idx);
...
int BNode::getSuccessor(int idx) {
	// Tiếp tục di chuyển về bên trái bắt đầu từ children[index + 1] cho tới khi
	// chúng ta chạm tới nút lá
...
void BNode::fill(int idx) {
	// Nếu nút con trước đó (children[idx - 1]) có nhiều hơn t - 1 khóa, mượn
	// một khóa từ nút con đó
	if (idx != 0 && this->children[idx - 1]->n >= this->t)
		this->borrowFromPrevious(idx);

	// Nếu nút con tiếp theo (children[idx + 1]) có nhiều hơn t - 1 khóa, mượn
	// một khóa từ nút con đó
	else if (idx != this->n && this->children[idx + 1]->n >= this->t)
		this->borrowFromNext(idx);

	// Ghép children[idx] với anh chị em của nó
	// Nếu children[idx] là nút con cuối cùng, ghép nó với anh chị em trước của
	// nó Nếu không thì ghép nó với anh chị em tiếp theo của nó
	else {
		if (idx != n)
			this->merge(idx);
		else
...
void BNode::borrowFromPrevious(int idx) {
	...
	// từ cha mẹ được chèm với chức trách là khóa đầu tiên trong children[idx].
	// Do đó chúng mất một khóa và nút con được nhận khóa đó

	// Di chuyển toàn bộ khóa trong children[idx] một bước vè phía trước
	for (int i = child->n - 1; i >= 0; --i)
		child->keys[i + 1] = child->keys[i];

	// Nếu children[idx] không phải là một nút là, di chuyển toàn bộ các con trỏ
	// con một bước về phía trước
	if (!child->isLeaf) {
	...
	// Di chuyển nút con cuối cùng của anh chị em bằng với nút con đầu tiên
	// children[idx]
	if (!child->isLeaf)
...
void BNode::borrowFromNext(int idx) {
	...
	// Nút con đầu tiên của anh chị em được chèn như là nút con cuối cùng của
	// children[idx]
	if (!child->isLeaf)
	...
	// Tăng và giảm số lượng khóa trong children[idx] và children[idx + 1] tương
	// ứng
	child->n++;
	sibling->n--;
...
// Hàm để gộp cây con thứ idx của nút với cây con thứ (idx+1) của nút (ngược với
// split)
void BNode::merge(int idx) {
	BNode* child = this->children[idx];
	BNode* sibling = this->children[idx + 1];

	// Đẩy một khóa từ nút current và chèn nó vào vị trí (t - 1)th của
	// children[idx]
	...
	// Di chuyển toàn bộ khóa đằng sau idx trong nút current một bước trước khi
	// điền đầy các khoảng trống được tạo ra để di chuyển keys[idx] tới
	// children[idx]
	for (int i = idx + 1; i < this->n; ++i)
		this->keys[i - 1] = this->keys[i];

	// Di chuyển các con trỏ con đằng sau (idx + 1) trong nút hiện tại một bước
	// về sau
...
void BTree::remove(int key) {
	if (!this->root) {
		cerr << "ERROR 002: Tree is empty\n";
		return;
	}

	// Gọi hàm xóa dánh cho root
	this->root->remove(key);

	// Nếu nút root không có khóa nào, làm cho nó trở thành nút con đầu tiển của
	// root mới nếu nó có một nút chon, nếu không thì chỉnh nó thành nullptr
	if (!this->root->n) {
		BNode* tmp = this->root;
		if (root->isLeaf)
			this->root = nullptr;
...
## `stdc++.hpp`
File này không cần thiết, hãy thay bằng `bits/stdc++.h` nếu bạn dùng GNU compiler
